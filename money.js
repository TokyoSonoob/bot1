// quickpay-addqr.js
const { Routes, ApplicationCommandOptionType, EmbedBuilder, Events, PermissionsBitField, ActionRowBuilder, StringSelectMenuBuilder } = require("discord.js");
const admin = require("firebase-admin");
const db = admin.firestore();

const TARGET_GUILD_ID = "1401622759582466229";
const TARGET_CHANNEL_ID = "1413522411025862799";
const COL = "quickpay_qr";
...
module.exports = function (client) {
  client.once("ready", async () => {
    try {
      ...
      const cmds = [
        {
          name: "addqr",
          description: "ตั้งค่าทริกเกอร์ (เช่น !nj) ให้ผูกกับรูป QR/ชำระเงิน แล้วให้บอทจำเพื่อนำไปส่ง Embed (แอดมินเท่านั้น)",
          dm_permission: false,
      ...
      await client.rest.put(Routes.applicationCommands(client.application.id), { body: cmds });
      console.log("✅ Registered /addqr, /removeqr (admin-only)");
    } catch (e) {
  ...
  client.on(Events.InteractionCreate, async (interaction) => {
    try {
      ...
      if (!member?.permissions?.has(PermissionsBitField.Flags.Administrator)) {
        return interaction.reply({ ephemeral: true, content: "❌ เฉพาะ **แอดมิน (Administrator)** เท่านั้นที่ใช้คำสั่งนี้ได้" });
      }
      ...
      if (!trigger.startsWith("!") || trigger.length < 2) {
        return interaction.reply({ ephemeral: true, content: "❌ โปรดใส่ trigger ที่ขึ้นต้นด้วย ! เช่น `!nj`" });
      }
      const timeoutAt = Date.now() + timeoutSec * 1000;
      awaitingImage.set(interaction.user.id, { trigger, timeoutAt, originChannelId: interaction.channelId });
  ...
  client.on(Events.InteractionCreate, async (interaction) => {
    try {
      ...
      if (interaction.isChatInputCommand() && interaction.commandName === "removeqr") {
        ...
        const embed = new EmbedBuilder().setTitle("เลือกรายการที่จะลบ").setDescription("เลือกทริกเกอร์จากเมนูด้านล่าง").setColor(0x9b59b6);
        const menu = new StringSelectMenuBuilder().setCustomId("qr:remove:select").setPlaceholder("เลือกทริกเกอร์").addOptions(options);
      ...
      console.error("removeqr interaction error:", e);
    }
  });
  ...
  client.on(Events.MessageCreate, async (message) => {
    try {
      ...
      if (pending) {
        if (now > pending.timeoutAt) {
          ...
          if (message.channelId === pending.originChannelId) {
            await message.reply({ content: "⏰ หมดเวลารอรูปแล้ว โปรดรัน `/addqr` ใหม่", allowedMentions: { repliedUser: false } });
          }
          return;
        }
        const firstImg =
          (message.attachments &&
            [...message.attachments.values()].find((a) => {
              const mimeOk = a.contentType?.startsWith?.("image/");
              const nameOk = /\.(png|jpe?g|gif|webp|bmp)$/i.test(a.name || "");
        ...
        if (!targetGuild) {
          awaitingImage.delete(message.author.id);
          await message.reply({ content: "❌ ไม่พบเซิร์ฟเวอร์ปลายทาง (TARGET_GUILD_ID) โปรดตรวจสอบการตั้งค่า", allowedMentions: { repliedUser: false } });
          return;
        }
        const targetChannel =
        ...
        if (!targetChannel || !targetChannel.isTextBased()) {
          awaitingImage.delete(message.author.id);
          await message.reply({ content: "❌ ไม่พบแชนเนลปลายทางหรือไม่ใช่แชนเนลข้อความ (TARGET_CHANNEL_ID)", allowedMentions: { repliedUser: false } });
          return;
        }
        const sent = await targetChannel.send({ content: `QR สำหรับ ${pending.trigger} (อัปโดย <@${message.author.id}>)`, files: [firstImg.url] });
        const imgAtt = sent.attachments?.first();
        const imgUrl = imgAtt?.url || firstImg.url;

        await db.collection(COL).doc(pending.trigger).set(
          {
        ...
        const confirm = await message.channel.send({ content: `✅ บันทึกเรียบร้อย! ผูกทริกเกอร์ \`${pending.trigger}\` กับรูปสำเร็จ` });
        setTimeout(() => {
      ...
      const doc = await db.collection(COL).doc(token).get();
      if (!doc.exists) return;

      if (
        message.guild &&
        message.channel &&
        message.channel.permissionsFor(message.client.user)?.has(PermissionsBitField.Flags.ManageMessages)
      ) {
        await message.delete().catch(() => {});
      }
