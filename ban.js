...
module.exports = (client) => {
  ...
  client.on(Events.InteractionCreate, async (interaction) => {
    ...
    const lines = [];
    lines.push(`🧹 ลบข้อความที่ตรวจพบ (รวมแชทในห้องเสียง/สเตจ): **${totalDeleted}** ข้อความ`);
    lines.push(disconnected ? "🔇 ตัดออกจากห้องเสียงแล้ว" : "🔇 ไม่ได้ตัดออกจากห้องเสียง (อาจไม่ได้อยู่/สิทธิ์ไม่พอ)");
    if (action === "banned") lines.push("⛔ ดำเนินการ: **แบน** สำเร็จ");
    else if (action === "kicked") lines.push("🚪 ดำเนินการ: **เตะออก** สำเร็จ");
    else lines.push("⚠️ ดำเนินการแบน/เตะไม่สำเร็จ (ตรวจสอบสิทธิ์บอทและลำดับบทบาท)");
