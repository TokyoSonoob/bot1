    ...
    try {
      ...
      if (ARTISTS.some((a) => a.id === i.customId)) {
        ...
        const map = {
          ...
          skin_yuu: ["ลายเส้นคุณยู", "สกินคุณยู", OWNER_IDS.skin_yuu],
        };
        ...
        if (existing.size >= 3) {
          await i.editReply(
            `❗ คุณสามารถเปิดตั๋ว ${skinName} ได้สูงสุด 3 ห้อง (ตอนนี้มี ${existing.size})`
          );
          return;
        }
        ...
        const embed = new EmbedBuilder()
          .setTitle(skinName)
          .setDescription(
            "สามารถเขียนข้อมูลหรือส่งรูปภาพไว้ได้เลยนะคับ รอแม่ค้ามาตอบ บรีฟไว้ได้เบย"
          )
          .setColor(0x9b59b6);
      ...
      try {
        if (!i.deferred && !i.replied) {
          await i.reply({
            content: "❌ อินเทอร์แอคชันหมดเวลา หรือเกิดข้อผิดพลาด ลองกดใหม่อีกครั้งน้า",
