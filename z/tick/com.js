...
module.exports = function (client) {
  const PREFIX = "!";
  const STAFF_ROLE_ID = "1438731715630858382";
  const CATEGORY_ID = "1438963309779878029";

  const OWNER_IDS = {
    com_muy: "1010202066720936048",
    com_nj: "1092393537238204497",
    com_ne: "765887179741200394",
  };
  ...
  async function safeReply(interaction, data, ephemeral = true) {
    const payload = { ...data };
    if (ephemeral) {
    ...
    try {
      ...
      console.error("com safeReply error:", err);
      try {
        if (!interaction.replied) {
          return await interaction.followUp({
            content: "❌ มีบางอย่างผิดพลาด ลองใหม่อีกครั้งน้า",
    ...
    try {
      ...
      if (command === "com") {
        if (
          ...
          await message.reply("❌ เฉพาะแอดมินเท่านั้นที่ใช้คำสั่งนี้ได้");
          return;
        }
        ...
        const row = new ActionRowBuilder().addComponents(
          new ButtonBuilder()
            .setCustomId("com_muy")
            .setLabel(LABELS.com_muy)
            .setStyle(ButtonStyle.Primary),
          new ButtonBuilder()
            .setCustomId("com_nj")
            .setLabel(LABELS.com_nj)
            .setStyle(ButtonStyle.Primary),
          new ButtonBuilder()
            .setCustomId("com_ne")
            .setLabel(LABELS.com_ne)
            .setStyle(ButtonStyle.Primary)
        );
      ...
      if (command === "closecom") {
        ...
        if (botMessage) {
          ...
          if (newButtons.length > 0) {
            ...
            await botMessage.edit({ components: [] }).catch(() => {});
          }
        }
        await message.delete().catch(() => {});
      }
      ...
      if (command === "opencom") {
        ...
        if (botMessage) {
          const currentRow = botMessage.components[0];
          const exists = currentRow.components.some(
            (btn) => btn.customId === customIdToAdd
          );
      ...
      console.error("com messageCreate error:", err);
    }
  });
  ...
  client.on("interactionCreate", async (interaction) => {
    ...
    try {
      ...
      if (interaction.customId === "delete_ticket") {
        if (interaction.channel?.parentId !== CATEGORY_ID) return;
        ...
        try {
          ...
          console.error("com delete_ticket error:", err);
        }
      }
    } catch (err) {
      console.error("com interactionCreate error:", err);
    }
  });
};
